...
function ValidationError (errors, statusCode) {
  ...
  this.status = 'ERROR';
  this.status_message = error.msg;
  if (/Missing/g.test(error.msg)) {
    this.statusCode = this.http_code = statusCode || 400;
    this.status_code = MISSING_INPUT;
  } else if (/Duplicate/g.test(error.msg)) {
    this.statusCode = this.http_code = statusCode || 400;
    this.status_code = RESOURCE_EXISTS;
  } else {
    // "Invalid" as default
    this.statusCode = this.http_code = statusCode || 400;
    this.status_code = INVALID_INPUT;
  }
}
...
function InternalError (error) {
  ...
  this.status = 'ERROR';
  this.status_code = EXTERNAL_FAILURE;
  this.status_message = error.message;
  this.http_code = this.statusCode = 500;
}
