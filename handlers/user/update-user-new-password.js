...
function checkUserCurrentPassword (req, res, next) {// eslint-disable-line id-length
  ...
  .catch(error => {
    ...
    req.log.error({
      err: error
    }, 'user.findOne Error - update-user-new-password');
  });
}
 ...
 * @returns {rpc} returns the validation error - failed response
 */
function updateUserNewPassword (req, res, next) {// eslint-disable-line id-length
  ...
  .catch(error => {
    ...
    req.log.error({
      err: error
    }, 'user.update Error - update-user-new-password');
  });
}
...
module.exports.logic = updateUserNewPassword;
